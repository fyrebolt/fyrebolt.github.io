// ===== Highlighter engine: compositing + timed highlight boxes + export =====

import { drawSource, drawHighlightBox, outputSizeFor } from '../render';
...
/** Plays the source straight through and draws each highlighter over its time range. */
export class HighlighterPlayer {
  ...
  private getState: () => HighlighterState;
  private onTime?: (sec: number) => void;

  constructor(canvas: HTMLCanvasElement, getState: () => HighlighterState, onTime?: (sec: number) => void) {
    this.canvas = canvas;
  ...
  private drawFrameAt(sec: number): void {
    ...
    for (const el of state.elements) {
      ...
      drawHighlightBox(this.ctx, this.out, el, sec);
    }
  }
  ...
  /** Build the export audio graph (video audio -> recording stream). */
  ...
  /** Topmost highlighter whose box contains a normalised point at the current time, or null. */
